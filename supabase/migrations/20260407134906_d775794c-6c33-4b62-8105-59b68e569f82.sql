...
DROP FUNCTION IF EXISTS public.get_invoice_summary(uuid);
...
BEGIN
  ...
  SELECT COUNT(*) INTO v_shipped_count FROM public.orders WHERE invoice_id = p_invoice_id AND delivery_status = 'shipped';
  ...
  SELECT COUNT(*) INTO v_invoice_confirmed_count FROM public.orders WHERE invoice_id = p_invoice_id AND confirmation_status = 'confirmed';

  v_confirmed_count := v_invoice_confirmed_count;
  ...
  WITH invoice_orders AS (
    SELECT o.id, o.order_id, o.customer_name, o.customer_phone, o.product_name, o.quantity, o.price, o.total_amount,
      ...
      COALESCE(p.weight_kg, CASE WHEN p.weight='up_to_1kg' THEN 0.5 WHEN p.weight='up_to_2kg' THEN 1.5 WHEN p.weight='up_to_3kg' THEN 2.5 WHEN p.weight='above_3kg' THEN 3.5 ELSE NULL END) AS weight_kg,
      EXISTS (SELECT 1 FROM public.invoice_adjustments ia WHERE ia.order_id = o.order_id AND ia.invoice_id = p_invoice_id) AS has_adjustment,
  ...
  -- Shipping breakdown (direct delivered only)
  WITH delivered AS (
    SELECT o.id, COALESCE(p.weight_kg, CASE WHEN p.weight='up_to_1kg' THEN 0.5 WHEN p.weight='up_to_2kg' THEN 1.5 WHEN p.weight='up_to_3kg' THEN 2.5 WHEN p.weight='above_3kg' THEN 3.5 ELSE 0.5 END)*o.quantity AS total_wt
    FROM public.orders o LEFT JOIN public.products p ON p.seller_id = o.seller_id AND p.name = o.product_name
    WHERE o.invoice_id = p_invoice_id AND o.delivery_status = 'delivered'
  ), brackets AS (
  ...
  SELECT COALESCE(SUM(CASE WHEN total_wt<=1 THEN v_rate_1kg WHEN total_wt<=2 THEN v_rate_2kg WHEN total_wt<=3 THEN v_rate_3kg ELSE v_rate_3kg_plus END),0) INTO v_shipping_fees
  FROM (SELECT COALESCE(p.weight_kg, CASE WHEN p.weight='up_to_1kg' THEN 0.5 WHEN p.weight='up_to_2kg' THEN 1.5 WHEN p.weight='up_to_3kg' THEN 2.5 WHEN p.weight='above_3kg' THEN 3.5 ELSE 0.5 END)*o.quantity AS total_wt
    FROM public.orders o LEFT JOIN public.products p ON p.seller_id = o.seller_id AND p.name = o.product_name
    WHERE o.invoice_id = p_invoice_id AND o.delivery_status = 'delivered') sub;

  -- Cross-invoice: ONLY delivered orders from closed invoices that were delivered AFTER closure
  WITH cross_delivered AS (
    SELECT o.id, o.order_id, o.customer_name, o.customer_phone, o.product_name, o.quantity, o.price, o.total_amount, o.created_at,
      o.delivery_status,
      COALESCE(p.weight_kg, CASE WHEN p.weight='up_to_1kg' THEN 0.5 WHEN p.weight='up_to_2kg' THEN 1.5 WHEN p.weight='up_to_3kg' THEN 2.5 WHEN p.weight='above_3kg' THEN 3.5 ELSE NULL END) AS weight_kg,
    ...
    WHERE o.seller_id = v_invoice.seller_id AND o.invoice_id != p_invoice_id
      ...
      AND EXISTS (SELECT 1 FROM public.order_history oh WHERE oh.order_id = o.order_id AND oh.field_changed = 'delivery_status' AND oh.new_value = 'delivered' AND oh.created_at > inv_orig.finalized_at)
  )
  SELECT COUNT(*),
    ...
    COALESCE(jsonb_agg(jsonb_build_object('id',id,'order_id',order_id,'customer_name',customer_name,'customer_phone',customer_phone,'product_name',product_name,'quantity',quantity,'price',price,'total_amount',total_amount,'created_at',created_at,'weight_kg',weight_kg,'total_weight_kg',COALESCE(weight_kg,0)*quantity,'amount_usd',ROUND(price*quantity/280.0,2),'confirmation_status','confirmed','delivery_status',delivery_status,'has_adjustment',false,'adjustment_invoice_id',NULL,'adjustment_invoice_number',NULL,'was_delivered',true,'is_cross_invoice',true,'original_invoice_number',original_invoice_number)),'[]'::jsonb)
  INTO v_cross_delivered_count, v_cross_delivered_orders, v_cross_orders FROM cross_delivered;

  v_cross_shipped_count := 0;

  -- Cross shipping fees: ONLY for delivered cross-invoice orders
  SELECT COALESCE(SUM(CASE WHEN COALESCE(weight_kg,0.5)*quantity<=1 THEN v_rate_1kg WHEN COALESCE(weight_kg,0.5)*quantity<=2 THEN v_rate_2kg WHEN COALESCE(weight_kg,0.5)*quantity<=3 THEN v_rate_3kg ELSE v_rate_3kg_plus END),0) INTO v_cross_shipping_fees
  FROM (SELECT o.quantity, COALESCE(p.weight_kg, CASE WHEN p.weight='up_to_1kg' THEN 0.5 WHEN p.weight='up_to_2kg' THEN 1.5 WHEN p.weight='up_to_3kg' THEN 2.5 WHEN p.weight='above_3kg' THEN 3.5 ELSE 0.5 END) AS weight_kg
    FROM public.orders o JOIN public.invoices inv_orig ON inv_orig.id = o.invoice_id LEFT JOIN public.products p ON p.seller_id = o.seller_id AND p.name = o.product_name
    WHERE o.seller_id = v_invoice.seller_id AND o.invoice_id != p_invoice_id AND inv_orig.status IN ('ready','paid') AND inv_orig.finalized_at IS NOT NULL
      AND o.delivery_status = 'delivered'
      AND EXISTS (SELECT 1 FROM public.order_history oh WHERE oh.order_id = o.order_id AND oh.field_changed = 'delivery_status' AND oh.new_value = 'delivered' AND oh.created_at > inv_orig.finalized_at)) sub;

  v_shipping_fees := v_shipping_fees + v_cross_shipping_fees;

  -- Cross delivered revenue
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object('id',id,'order_id',order_id,'seller_id',seller_id,'invoice_id',invoice_id,'applied_invoice_id',applied_invoice_id,'old_status',old_status,'new_status',new_status,'difference',difference,'difference_usd',ROUND(difference/280.0,2),'reason',reason,'status',status,'created_at',created_at)),'[]'::jsonb),
    COALESCE(SUM(CASE WHEN status='approved' THEN difference ELSE 0 END),0)
  ...
  RETURN jsonb_build_object(
    ...
    'counts', jsonb_build_object('total_orders_count',v_total_orders_count,'delivered_count',v_delivered_count,'shipped_count',v_shipped_count,'confirmed_count',v_confirmed_count,'dropped_count',v_dropped_count,'cross_shipped_count',v_cross_shipped_count,'cross_delivered_count',v_cross_delivered_count),
