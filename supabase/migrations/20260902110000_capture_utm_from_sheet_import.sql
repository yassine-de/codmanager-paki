-- Capture the "UTM Source" column from the seller's Google Sheet (which
-- already exists there, e.g. "facebook"/"tiktok" per order) into the
-- existing, previously-unused orders.source_ref column, so the admin can
-- see/filter which channel each imported order actually came from — needed
-- to check whether orders are still arriving from ad-excluded regions/
-- channels after a location exclusion is set up.
CREATE OR REPLACE FUNCTION public.create_sheet_order_with_items(p_order jsonb, p_items jsonb)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- total_price is GENERATED ALWAYS from quantity * unit_price.
