...
BEGIN
  ...
  -- Cross-invoice shipped (with fix: exclude orders already shipped before closed invoice finalization)
  ...
  -- NEW: Cross-invoice delivered-only (re-delivered after close, NOT captured by cross_shipped)
  WITH cross_delivered_only AS (
    ...
    WHERE o.seller_id = v_invoice.seller_id
      ...
      -- Exclude orders already captured via cross_shipped (newly shipped after close)
      AND NOT EXISTS (
  ...
  -- Cross-delivered revenue now comes from the dedicated CTE (no standalone query)
