-- Fix: for a seller's first invoice (no previous finalized invoice),
-- v_period_start was incorrectly set to invoice.created_at, which excluded
-- all orders confirmed/delivered before the invoice was created.
-- Now v_period_start = '-infinity' for first invoices → all history is included.
...
BEGIN
  ...
  -- SHIPPED COUNT (NEW RULE): any order with a 'shipped' event in period.
  ...
  -- Cross-invoice shipped (NEW RULE: existence of 'shipped' event in period, no last-status filter)
  ...
  -- Cross-invoice delivered-only (unchanged)
  ...
  -- Shipping breakdown (NEW RULE): bill any order with a 'shipped' event in period.
  ...
  -- Combine cross-delivered revenue from BOTH paths
