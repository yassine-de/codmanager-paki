...
BEGIN
  ...
  IF OLD.delivery_status IS DISTINCT FROM NEW.delivery_status THEN
    ...
    -- ONLY create revenue adjustment if order WAS ALREADY DELIVERED in the closed invoice
    -- If it was just shipped (not delivered), this is a new event → no adjustment needed
    IF OLD.delivery_status = 'delivered' AND NEW.delivery_status != 'delivered' THEN
      -- Was delivered in closed invoice, now reverted → negative adjustment
    ...
    ELSIF OLD.delivery_status = 'delivered' AND NEW.delivery_status = 'delivered' THEN
      -- No change in delivered state, skip
      NULL;
    END IF;

    -- NOTE: If OLD.delivery_status != 'delivered' (e.g. shipped→delivered),
    -- we do NOT create an adjustment. The auto_assign_invoice_on_delivery trigger
    -- will move the order to the open invoice where it belongs as new revenue.
  ...
  THEN
    v_diff := 0;
    -- Revenue impact only if order is currently delivered
