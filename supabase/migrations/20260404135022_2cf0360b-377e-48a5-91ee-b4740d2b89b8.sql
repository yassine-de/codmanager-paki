...
BEGIN
  ...
  WITH invoice_orders AS (
    SELECT o.id, o.order_id, o.customer_name, o.customer_phone, o.product_name, o.quantity, o.price, o.total_amount, o.confirmation_status, o.delivery_status, o.created_at, p.weight_kg,
  ...
  WITH invoice_orders AS (
    SELECT o.id, o.order_id, o.customer_name, o.customer_phone, o.product_name, o.quantity, o.price, o.total_amount, o.created_at, p.weight_kg
    FROM public.orders o
    LEFT JOIN public.products p ON p.seller_id = o.seller_id AND p.name = o.product_name
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object('id', id, 'order_id', order_id, 'seller_id', seller_id, 'invoice_id', invoice_id, 'applied_invoice_id', applied_invoice_id, 'old_status', old_status, 'new_status', new_status, 'difference', difference, 'difference_usd', difference / 290.0, 'reason', reason, 'status', status, 'created_at', created_at) ORDER BY created_at ASC), '[]'::jsonb),
  ...
  v_call_center_fees := (COALESCE(v_confirmed_count, 0) * COALESCE(v_confirmed_rate, 0)) + (COALESCE(v_dropped_count, 0) * COALESCE(v_dropped_rate, 0));
  ...
  RETURN jsonb_build_object(
    ...
    'counts', jsonb_build_object('total_orders_count', COALESCE(v_total_orders_count, 0), 'delivered_count', COALESCE(v_delivered_count, 0), 'shipped_count', COALESCE(v_shipped_count, 0), 'confirmed_count', COALESCE(v_confirmed_count, 0), 'dropped_count', COALESCE(v_dropped_count, 0)),
    'call_center_breakdown', jsonb_build_object('confirmed_count', COALESCE(v_confirmed_count, 0), 'confirmed_rate', COALESCE(v_confirmed_rate, 0), 'confirmed_fees', COALESCE(v_confirmed_count, 0) * COALESCE(v_confirmed_rate, 0), 'dropped_count', COALESCE(v_dropped_count, 0), 'dropped_rate', COALESCE(v_dropped_rate, 0), 'dropped_fees', COALESCE(v_dropped_count, 0) * COALESCE(v_dropped_rate, 0)),
