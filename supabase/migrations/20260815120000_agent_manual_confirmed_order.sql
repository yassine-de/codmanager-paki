-- Let create_manual_order_with_items accept delivery_status / confirmed_at / agent_id
-- so a confirmation agent can create an order that is immediately confirmed & booked
-- (used by the new "Add New Order" button on the agent Process Orders screen).
-- Also fixes changed_by_role on the resulting order_history row: it previously
-- collapsed every staff caller (admin, agent, warehouse...) down to 'admin' via
-- is_staff(), which mislabeled agent-created orders in the Order History UI.
