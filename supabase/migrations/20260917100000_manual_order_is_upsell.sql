-- create_manual_order_with_items() never set orders.is_upsell — so when an
-- agent creates a brand-new order (not confirming an existing one) with
-- quantity > 1, it never counted as an upsell anywhere upsell is tracked
-- (unlike agent_submit_order()'s confirm flow, which already flags
-- is_upsell when the confirmed quantity exceeds the order's original
-- quantity). Adds is_upsell to the insert, sourced from p_order — the
-- frontend now sends it as (total quantity > 1).
...
BEGIN
  ...
  VALUES (
    ...
    CASE WHEN NULLIF(p_order ->> 'agent_id', '') IS NOT NULL THEN (p_order ->> 'agent_id')::uuid ELSE NULL END,
    COALESCE((p_order ->> 'is_upsell')::boolean, false)
  )
