-- Add partial unique index: only one open invoice per seller
CREATE UNIQUE INDEX idx_one_open_invoice_per_seller 
ON public.invoices (seller_id) 
WHERE status = 'open';

-- Update trigger with advisory lock to prevent race conditions
...
BEGIN
  ...
  IF TG_OP = 'INSERT' THEN
    IF NEW.invoice_id IS NULL THEN
      -- Advisory lock on seller_id to serialize invoice creation
