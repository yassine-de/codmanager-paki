-- Fix confirmed count: trust orders.confirmation_status as source of truth.
-- Previous logic relied on order_history which is sometimes incomplete
-- (orders updated directly in DB have no matching history entry).
-- Now uses confirmed_at (if set) or updated_at for date-bounding.
...
BEGIN
  ...
  -- Find the most recent finalized invoice before this one for the same seller.
  -- If none exists (first invoice), use '-infinity' so all history is included.
  ...
  SELECT COUNT(DISTINCT o.id) INTO v_total_orders_count FROM public.orders o WHERE o.invoice_id = p_invoice_id;

  -- DELIVERED COUNT: time-bounded via order_history
  ...
  -- SHIPPED COUNT: any order with a 'shipped' event in period.
  SELECT COUNT(DISTINCT o.id) INTO v_shipped_count
  FROM public.orders o
  WHERE o.invoice_id = p_invoice_id
    AND EXISTS (
      SELECT 1 FROM public.order_history oh
  ...
  -- DROPPED COUNT
  SELECT COUNT(DISTINCT o.id) INTO v_dropped_count
  FROM public.orders o
  WHERE o.seller_id = v_invoice.seller_id
    AND o.created_at > v_period_start
    AND o.created_at <= v_period_end;

  -- CONFIRMED COUNT (direct):
  -- Trust orders.confirmation_status as source of truth.
  -- Use confirmed_at if set, otherwise updated_at for date-bounding.
  -- This handles orders confirmed directly in DB without order_history entries.
  SELECT COUNT(DISTINCT o.id) INTO v_invoice_confirmed_count
  FROM public.orders o
  WHERE o.invoice_id = p_invoice_id
  ...
  -- CONFIRMED COUNT (cross-invoice):
  -- Orders from other finalized invoices confirmed during this period.
  -- Uses confirmed_at if set (safe), else falls back to order_history (NOT updated_at
  -- to avoid double-counting orders whose updated_at changed due to delivery updates).
  SELECT COUNT(DISTINCT o.id) INTO v_cross_confirmed_count
  FROM public.orders o
  ...
  WHERE o.seller_id = v_invoice.seller_id
    ...
    AND o.confirmation_status = 'confirmed'
    AND (
      -- Option 1: confirmed_at is set — safest, no duplication risk
      ...
      OR
      -- Option 2: no confirmed_at — use order_history (NOT updated_at)
  ...
  -- Delivered orders (direct) — time-bounded
  ...
  -- All orders with was_delivered — time-bounded
  WITH invoice_orders AS (
  ...
  -- Revenue from direct delivered — time-bounded
  SELECT COALESCE(SUM(ROUND(o.price*o.quantity/v_pkr_rate,2)),0) INTO v_delivered_revenue_usd
  FROM public.orders o
  WHERE o.invoice_id = p_invoice_id AND o.delivery_status = 'delivered'
    AND EXISTS (
      SELECT 1 FROM public.order_history oh
  ...
  WITH cross_shipped AS (
    SELECT o.id, o.order_id, o.customer_name, o.customer_phone, o.product_name, o.quantity, o.price, o.total_amount, o.created_at,
      ...
      (o.delivery_status = 'delivered' AND EXISTS (
        SELECT 1 FROM public.order_history oh_del
  ...
  WITH direct_shipped AS (
    SELECT DISTINCT o.id,
      COALESCE(p.weight_kg, CASE WHEN p.weight='up_to_1kg' THEN 0.5 WHEN p.weight='up_to_2kg' THEN 1.5 WHEN p.weight='up_to_3kg' THEN 2.5 WHEN p.weight='above_3kg' THEN 3.5 ELSE 0.5 END)*o.quantity AS total_wt
    FROM public.orders o LEFT JOIN public.products p ON p.seller_id = o.seller_id AND p.name = o.product_name
    WHERE o.invoice_id = p_invoice_id
      AND EXISTS (
        SELECT 1 FROM public.order_history oh
        WHERE oh.order_id = o.order_id AND oh.field_changed = 'delivery_status' AND oh.new_value = 'shipped'
          AND oh.created_at > v_period_start AND oh.created_at <= v_period_end
      )
  ), cross_shipped_wt AS (
    SELECT DISTINCT o.id,
      COALESCE(p.weight_kg, CASE WHEN p.weight='up_to_1kg' THEN 0.5 WHEN p.weight='up_to_2kg' THEN 1.5 WHEN p.weight='up_to_3kg' THEN 2.5 WHEN p.weight='above_3kg' THEN 3.5 ELSE 0.5 END)*o.quantity AS total_wt
    ...
    WHERE o.seller_id = v_invoice.seller_id AND o.invoice_id != p_invoice_id
      ...
      AND NOT EXISTS (
        ...
        WHERE oh_prev.order_id = o.order_id
          ...
          AND oh_prev.created_at = (
            SELECT MAX(created_at) FROM public.order_history
            WHERE order_id = o.order_id AND field_changed = 'delivery_status' AND created_at <= inv_orig.finalized_at
          )
          AND oh_prev.new_value IN ('shipped','in_transit','with_courier','delivered','returned')
      )
  ), all_shipped AS (
