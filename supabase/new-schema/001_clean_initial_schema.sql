...
AS $$
  ...
  LEFT JOIN public.profiles sp ON sp.user_id = o.seller_id
  LEFT JOIN public.profiles ap ON ap.user_id = o.agent_id
  WHERE o.delivery_status IN ('shipped','in_transit','out_for_delivery','with_courier','delivered','paid','failed_attempt','returned','return','ready_for_return','return_received')
...
AS $$
  ...
  FROM public.orders
  WHERE delivery_status IN ('shipped','in_transit','out_for_delivery','with_courier','delivered','paid','failed_attempt','returned','return','ready_for_return','return_received');
$$;
...
CREATE POLICY "Sellers insert own orders" ON public.orders FOR INSERT TO authenticated WITH CHECK (seller_id = auth.uid());
...
CREATE POLICY "Sellers manage own sourcing requests" ON public.sourcing_requests FOR ALL TO authenticated USING (seller_id = auth.uid()) WITH CHECK (seller_id = auth.uid());
CREATE POLICY "Sellers view own sourcing history" ON public.sourcing_history FOR SELECT TO authenticated USING (EXISTS (SELECT 1 FROM public.sourcing_requests sr WHERE sr.id = public.sourcing_history.sourcing_request_id AND sr.seller_id = auth.uid()));
CREATE POLICY "Sellers view own invoices" ON public.invoices FOR SELECT TO authenticated USING (seller_id = auth.uid());
CREATE POLICY "Sellers view own invoice items" ON public.invoice_items FOR SELECT TO authenticated USING (EXISTS (SELECT 1 FROM public.invoices i WHERE i.id = public.invoice_items.invoice_id AND i.seller_id = auth.uid()));
CREATE POLICY "Sellers view own invoice addons" ON public.invoice_addons FOR SELECT TO authenticated USING (EXISTS (SELECT 1 FROM public.invoices i WHERE i.id = public.invoice_addons.invoice_id AND i.seller_id = auth.uid()));
CREATE POLICY "Sellers view own invoice history" ON public.invoice_history FOR SELECT TO authenticated USING (EXISTS (SELECT 1 FROM public.invoices i WHERE i.id = public.invoice_history.invoice_id AND i.seller_id = auth.uid()));
CREATE POLICY "Sellers view own invoice adjustments" ON public.invoice_adjustments FOR SELECT TO authenticated USING (seller_id = auth.uid());
CREATE POLICY "Sellers manage own support tickets" ON public.support_tickets FOR ALL TO authenticated USING (seller_id = auth.uid()) WITH CHECK (seller_id = auth.uid());
CREATE POLICY "Sellers view own support messages" ON public.support_messages FOR SELECT TO authenticated USING (EXISTS (SELECT 1 FROM public.support_tickets st WHERE st.id = public.support_messages.ticket_id AND st.seller_id = auth.uid()));
CREATE POLICY "Sellers insert own support messages" ON public.support_messages FOR INSERT TO authenticated WITH CHECK (sender_id = auth.uid() AND EXISTS (SELECT 1 FROM public.support_tickets st WHERE st.id = public.support_messages.ticket_id AND st.seller_id = auth.uid()));
